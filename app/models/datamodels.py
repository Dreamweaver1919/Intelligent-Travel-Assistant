...
class DebugInfo(BaseModel):
    """生成过程诊断信息"""
    generation_source: str = Field(default="unknown", description="agent_mcp/fallback")
    failure_stage: Optional[str] = Field(default=None, description="失败环节")
    failure_reason: Optional[str] = Field(default=None, description="失败原因")
    details: Optional[str] = Field(default=None, description="原始错误摘要")
...
class TripPlan(BaseModel):
    ...
    generation_source: str = Field(default="unknown",description="生成来源")
    debug_info: Optional[DebugInfo] = Field(default=None,description="调试信息")

class TripPlanRequest(BaseModel):
    city: str = Field(..., description="目的地城市")
    start_date: str = Field(..., description="旅行开始日期")
    end_date: str = Field(..., description="旅行结束日期")
    days: int = Field(default=3, description="旅行天数")
    preferences: str = Field(..., description="旅行偏好，如：历史文化、自然风光")
    budget: str = Field(..., description="预算要求，如：经济、中等、奢华")
    transportation: str = Field(..., description="交通方式")
